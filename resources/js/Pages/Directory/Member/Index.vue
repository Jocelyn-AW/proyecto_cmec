...
const uploadAvatar = (event) => {
    ...
    router.post(route('directory.profile', props.directory.id), formData, {
        ...
        preserveScroll: true,
        onSuccess: () => {
            success('Foto actualizada con exito.')
            event.target.value = null
        },
        onError: (errors) => {
            console.error('Hubo un error al subir el archivo:', errors)
            errorA('Hubo un problema al actualizar la foto de perfil.')
        }
    })
}
            ...
            <div class="flex flex-col lg:flex-row">
                ...
                <div class="flex flex-1 flex-col justify-center p-8 lg:p-10">
                    ...
                    <p class="mt-3 max-w-md text-sm leading-relaxed text-gray-500">
                        Gestiona los datos de tu directorio. Puedes actualizar su información y subir nuevo archivos en
                        cualquier momento.
                    </p>
                    <div class="mt-5">
                        <span v-if="isActive"
                ...
                <div class="relative h-40 w-full lg:h-auto lg:w-64 xl:w-72 overflow-hidden">
                    ...
                    <div class="absolute inset-0 flex items-center justify-center">
                        <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink"
                            stroke="currentColor" viewBox="0 0 20 20" stroke-width=".1" stroke-linecap="round"
                            stroke-linejoin="round" class="h-50 text-brand-200 dark:text-brand-800">
                            <g fill="none">
                                <path
                    ...
                    <div class="mt-6 flex flex-col items-center justify-around gap-4 pl-8 pb-8">
                        ...
                        <label class="relative cursor-pointer group">
                            <div
                                ...
                                <img v-if="directory.profile_url" :src="directory.profile_url"
                        ...
                        <div class="grid grid-cols-2 gap-3">
                            <div>
                                ...
                                <select name="state" id="state" v-model="selectedState"
                                    ...
                                    <option v-for="state in Object.keys(states)" :key="state" :value="state">{{ state }}
                                    </option>
                                </select>
                                <span v-if="errors?.state" class="grow text-red-500 text-xs flex justify-end">{{
                                    errors?.state }}</span>
                            </div>
                            <div>
                                <label
                                ...
                                <span v-if="errors?.city" class="grow text-red-500 text-xs flex justify-end">{{
                                    errors?.city }}</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div v-for="(clinic, index) in formData.clinics" class="space-y-6 mt-4">
                        <div class="grid grid-cols-3 gap-3">
                            ...
                            <div>
                                ...
                                <input type="text" v-model="clinic.hospital_name" maxlength="190" placeholder="Ej: Hospital General No. 16"
                                    class="dark:bg-dark-900 h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 shadow-theme-xs placeholder:text-gray-400 focus:border-brand-300 focus:outline-hidden focus:ring-3 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30 dark:focus:border-brand-800" />
                                <span v-if="errors[`clinics.${index}.hospital_name`]"
                                    class="text-red-500 text-xs flex justify-end mt-1">{{
                                        errors[`clinics.${index}.hospital_name`] }}</span>
                            </div>
                            <!-- Direccion -->
                            <div class="col-span-2">
                                <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">Dirección</label>
                                <input type="text" v-model="clinic.address" maxlength="190" placeholder="Ej: Av. Cuauhtémoc 123, Col. Centro, CDMX"
                                    class="dark:bg-dark-900 h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 shadow-theme-xs placeholder:text-gray-400 focus:border-brand-300 focus:outline-hidden focus:ring-3 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30 dark:focus:border-brand-800" />
                                <span v-if="errors[`clinics.${index}.address`]"
                                    class="text-red-500 text-xs flex justify-end mt-1">{{
                                        errors[`clinics.${index}.address`] }}</span>
                            </div>
                            <!-- Telefono -->
                            <div class="col-start-1">
                                <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">Teléfono de contacto</label>
                                <input type="text" v-model="clinic.phone" maxlength="12" placeholder="Ej: 55 125-123-00"
                                    class="dark:bg-dark-900 h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 shadow-theme-xs placeholder:text-gray-400 focus:border-brand-300 focus:outline-hidden focus:ring-3 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30 dark:focus:border-brand-800" />
                                <span v-if="errors[`clinics.${index}.phone`]"
                                    class="text-red-500 text-xs flex justify-end mt-1">{{
                                        errors[`clinics.${index}.phone`] }}</span>
                            </div>
                            <!-- Horario -->
                            <div class="col-span-2">
                                <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">Horario</label>
                                <input type="text" v-model="clinic.schedule" maxlength="190" placeholder="Ej: Lunes a viernes de 9:00 a 18:00"
                                    class="dark:bg-dark-900 h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 shadow-theme-xs placeholder:text-gray-400 focus:border-brand-300 focus:outline-hidden focus:ring-3 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30 dark:focus:border-brand-800" />
                                <span v-if="errors[`clinics.${index}.schedule`]"
                                    class="text-red-500 text-xs flex justify-end mt-1">{{
                                        errors[`clinics.${index}.schedule`] }}</span>
                            </div>
                            <div class="col-span-3 flex justify-end">
                                <button type="button" class="top-3 text-red-400 hover:text-red-600"
                                    v-if="formData.clinics.length > 1" @click="removeClinic(index)">
