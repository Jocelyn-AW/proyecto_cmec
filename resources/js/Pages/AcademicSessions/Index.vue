...
const openPdf = (session) => {
    if (session.program_url) {
        window.open(session.program_url, '_blank');
    } else {
        warning('Esta sesión académica no tiene un programa PDF asociado.', {
            title: 'Sin programa',
            buttonText: 'Aceptar'
        })
    }
...
const openGallery = (session) => {
    ...
    router.get(route('albums.index', {
        event_type: session.sessions[0].sessionable_type,
        event_id: session.id,
    }))
}
</script>
...
<template>
    ...
    <div class="p-6 border-t border-gray-100 dark:border-gray-800 sm:p-6">
        <div class="space-y-5">
            ...
            <div>
                <h3 class="text-lg font-semibold text-gray-800 dark:text-white/90">Sesiones Académicas</h3>
                <p class="text-sm text-gray-500">Administra las sesiones académicas de tu aplicación desde esta sección
                </p>
            </div>
