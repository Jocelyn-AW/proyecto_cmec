    ...
    <Drawer :show="show" title="Editar asistente" :subtitle="props.eventName" size="xl" @close="emit('close')">
        <div class="space-y-4">
            ...
            <div v-if="isConference">
                ...
                <select name="person_type" id="person_type" v-model="createForm.person_type" :disabled="isPayed"
            ...
            <div v-if="createForm.person_type == 'member'">
                ...
                <div class="flex gap-2">
                    <input v-model="createForm.cmec_member_id" type="text" placeholder="Ej. CMEC-0001"
                        ...
                        @keyup.enter="fetchMemberData" :disabled="isPayed"/>
                    <button type="button" @click="fetchMemberData"
                        :disabled="isLoadingMember || !createForm.cmec_member_id?.trim() || isPayed"
            ...
            <div>
                ...
                <div class="flex gap-2 w-full">
                    <input :value="price" :disabled="isPayed || isFree" :class="isPayed || isFree ? 'disabled' : ''"
                        type="number" min="0" step="0.01"
                        class="grow rounded-lg border border-gray-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500"
                        placeholder="Cantidad a pagar" />
                    <select name="method" id="method" v-model="createForm.payment_method" :disabled="isPayed || isFree"
                        :class="isPayed || isFree ? 'disabled' : ''"
                ...
                <div class="flex gap-2 w-full mt-3">
                    <select name="status" id="status" v-model="createForm.status" :disabled="isPayed || isFree"
                        :class="isPayed || isFree ? 'disabled' : ''"
                        class="rounded-lg grow border border-gray-300 px-3 py-2 text-sm text-gray-700 focus:outline-none focus:ring-2 focus:ring-indigo-500">
