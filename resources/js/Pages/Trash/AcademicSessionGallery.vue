...
const submitImages = () => {
    ...
    router.post(route('academicsessions.gallery.update', props.academicsession.id), form, {
        forceFormData: true,
        onSuccess: () => {
            reset()
...
const deleteImage = (mediaId) => {
    hideAlert()
    router.delete(route('academicsessions.gallery.delete', {
        id: props.academicsession.id,
        mediaId: mediaId
    }), {
...
<template>
    ...
    <Head title="Galería de la Sesión Academica" />
    <div class="p-6 border-t border-gray-100 dark:border-gray-800 sm:p-6">
        <div class="space-y-5">
            <div class="">
                <h3 class="text-lg font-semibold text-gray-800 dark:text-white/90">Galería de la Sesión Academica: {{ academicsession.topic
                    }}
                </h3>
            ...
            <div @keydown="onKeydown" tabindex="-1">
                ...
                <Alerta v-bind="alertState" @confirm="alertState.onConfirm ? alertState.onConfirm() : hideAlert()"
                    @cancel="alertState.onCancel ? alertState.onCancel() : hideAlert()" @close="hideAlert()" />

                <!-- Header -->
                <div class="flex items-center justify-between mb-6">
                    <div class="flex items-center gap-3">
                        <button @click="router.get(route('academicsessions.index'))" class="text-gray-500 hover:text-gray-700">
                            ← Volver
                        </button>
                    </div>
