...
const handleOnCreate = () => { showCreateDrawer.value = true }
...
const onChangeAttend = (attendee) => {
    router.get(
        route('attendees.change-attend', attendee.id),
            ...
            <DataTable :columns="[
                ...
                <template #filters>
                    <div class="flex flex-wrap items-center gap-3 px-4 py-3">
                        <label class="whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">Webinar</label>
                        ...
                        <label class="whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">Asistencia</label>
                ...
                <template #cell-status="{ item }">
                    <span role="button" @click="openPaymentDetails(item)"
                        ...
                        :class="item.status === 'pending' ? 'bg-amber-200 text-amber-700 hover:bg-amber-300' :
                            item.status === 'paid' ? 'bg-emerald-200 text-emerald-700 hover:bg-emerald-300' :
                ...
                <template #actionButtons="{ item }">
                    <button :title="item.diploma_url ? 'Ver diploma PDF' : 'Subir diploma'" @click="openDiploma(item)"
                        :disabled="!item.did_attend" :class="[
                            item.diploma_url
