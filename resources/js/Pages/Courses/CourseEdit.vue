...
const currentPdf = computed(() => {
    ...
    } else if (props.course?.program_url) {
        return {
            type: 'application/pdf',
            isFile: true,
            size: null,
            name: props.course?.program_url.split('/').pop(),
        }
    } else {
        return null;
    }
...
const fillForm = (course) => {
    formData.id = course.id || null;
    formData.topic = course.topic || "";
    formData.description = course.description || "";
    formData.objectives = course.objectives || "";
    formData.date = course.date || "";
    formData.time = getTimeFromDateTime(course.date) || "";
    formData.duration = course.duration || "";
    formData.organized_by = course.organized_by || "";
    formData.link = course.link || "";
        ...
        <div class="space-y-5">
            <div class="">
                ...
                <p class="text-sm text-gray-500">
                    Edita los detalles de un curso desde esta sección
                </p>
            </div>
                ...
                <div class="grid lg:grid-cols-4 gap-4 p-6">
                    ...
                    <div class="lg:col-span-3 space-y-6">
                        ...
                        <div>
                            ...
                            <textarea
                                v-model="formData.objectives" rows="3" maxlength="2005"
                                class="dark:bg-dark-900  w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 shadow-theme-xs placeholder:text-gray-400 focus:border-brand-300 focus:outline-hidden focus:ring-3 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30 dark:focus:border-brand-800"
                            />
                            
                            <p class="mt-1 text-xs text-gray-400 text-right">{{ formData.objectives.length }}/2000</p>
            ...
            >
                <div class="grid grid-cols-4 gap-4 p-6">
                    ...
                    <div class="col-span-2 flex justify-end items-center">
                        ...
                        <button 
                            @click="handleSubmit"
                            :class="props.course.deleted_at ? 'disabled border border-gray-600' : ''" 
